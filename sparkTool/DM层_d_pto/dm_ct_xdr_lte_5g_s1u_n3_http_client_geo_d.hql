...
drop table if exists dm_ct_xdr_lte_5g_s1u_n3_http_client_geo_d;
create EXTERNAL table IF NOT EXISTS dm_ct_xdr_lte_5g_s1u_n3_http_client_geo_d(
`starttime` timestamp,
`geo_id` int,
`ne_type` int,
`province_id` int,
...
location "${hivevar:basepath}/dm_ct_xdr_lte_5g_s1u_n3_http_client_geo_d/";