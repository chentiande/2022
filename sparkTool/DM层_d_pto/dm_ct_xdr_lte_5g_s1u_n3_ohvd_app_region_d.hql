...
drop table if exists dm_ct_xdr_lte_5g_s1u_n3_ohvd_app_region_d;
create EXTERNAL table IF NOT EXISTS dm_ct_xdr_lte_5g_s1u_n3_ohvd_app_region_d(
`starttime` timestamp,
`province_id` int,
...
`n3_game_cnt` bigint,
`game_tcp_avg_total_delay` bigint,
`game_tcp_avg_total_delay_cnt` bigint,
`game_udp_avg_total_delay` bigint,
`game_udp_avg_total_delay_cnt` bigint,
`game_tcp_avg_server_good` bigint,
`game_tcp_avg_server_bat` bigint,
`game_udp_avg_server_good` bigint,
`game_udp_avg_server_bat` bigint,
`game_tcp_avg_delay` decimal(24,4),
`game_udp_avg_delay` decimal(24,4),
`game_tcp_avg_server_good_rate` decimal(12,6),
`game_udp_avg_server_good_rate` decimal(12,6),
`game_avg_server_good_rate` decimal(12,6)) 
PARTITIONED BY ( 
  `p_date` string)
ROW FORMAT DELIMITED 
  FIELDS TERMINATED BY '|' NULL DEFINED AS '' 
location "${hivevar:basepath}/dm_ct_xdr_lte_5g_s1u_n3_ohvd_app_region_d/";