...
drop table if exists dm_ct_xdr_lte_5g_s1u_n3_ohvd_usr_cell_h;
create EXTERNAL table IF NOT EXISTS dm_ct_xdr_lte_5g_s1u_n3_ohvd_usr_cell_h(
`starttime` timestamp,
`tac` int,
`ncgi_ecgi` varchar(16),
`nci_eci` bigint,
`nc_ec_id` int,
`cell_name` varchar(255),
`gnb_enb_id` int,
`gnb_enb_name` varchar(128),
`gnb_enb_ip` varchar(200),
`province_id` int,
...
location "${hivevar:basepath}/dm_ct_xdr_lte_5g_s1u_n3_ohvd_usr_cell_h/";