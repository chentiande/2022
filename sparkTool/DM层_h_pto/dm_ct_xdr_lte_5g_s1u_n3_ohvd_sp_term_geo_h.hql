...
drop table if exists dm_ct_xdr_lte_5g_s1u_n3_ohvd_sp_term_geo_h;
create EXTERNAL table IF NOT EXISTS dm_ct_xdr_lte_5g_s1u_n3_ohvd_sp_term_geo_h(
`starttime` timestamp,
`geo_id` int,
`ne_type` int,
`province_id` int,
...
location "${hivevar:basepath}/dm_ct_xdr_lte_5g_s1u_n3_ohvd_sp_term_geo_h/";